...
const MONTHS_ORDER = ["Setembro","Outubro","Novembro","Dezembro","Janeiro","Fevereiro","Março","Abril","Maio","JUNHO","JULHO","AGOSTO","SETEMBRO","OUTUBRO","NOVEMBRO","DEZEMBRO"];
...
function CalendarioMensal({ data, selectedMonth, allMonths, consultores, clientColors, onEdit, onDelete, onNewEntry, readonly }) {
  ...
  const now = new Date();
  const guessYear = monthNum <= now.getMonth() ? now.getFullYear() + 1 : now.getFullYear();
...
// role: "admin" → acesso total | "editor" → edita agenda, sem cadastros
//       "viewer" → só visualiza | "consultor" → só vê sua própria agenda
// ─────────────────────────────────────────────────────────────────────────────
const USERS = [
  { username:"admin",    password:"admin123",   role:"admin",     label:"Administrador",  consultorName:null },
  { username:"editor",   password:"editor123",  role:"editor",    label:"Editor",         consultorName:null },
  { username:"viewer",   password:"viewer123",  role:"viewer",    label:"Visualizador",   consultorName:null },
  // Consultores — username = primeiro nome em minúsculas, senha = nome+ano
  { username:"antonio",  password:"antonio2025",role:"consultor", label:"Consultor",      consultorName:"Antonio Matos" },
  { username:"augusto",  password:"augusto2025",role:"consultor", label:"Consultor",      consultorName:"Augusto Meirelles" },
  { username:"celso",    password:"celso2025",  role:"consultor", label:"Consultor",      consultorName:"Celso Tarabori" },
  { username:"dalcione", password:"dalcione2025",role:"consultor",label:"Consultor",      consultorName:"Dalcione Carpenedo" },
  { username:"dirce",    password:"dirce2025",  role:"consultor", label:"Consultor",      consultorName:"Dirce Matos" },
  { username:"hilton",   password:"hilton2025", role:"consultor", label:"Consultor",      consultorName:"Hilton Vinhola" },
  { username:"lucas",    password:"lucas2025",  role:"consultor", label:"Consultor",      consultorName:"Lucas Cintra" },
  { username:"rodrigo",  password:"rodrigo2025",role:"consultor", label:"Consultor",      consultorName:"Rodrigo Rodrigues" },
  { username:"fabi",     password:"fabi2025",   role:"consultor", label:"Consultor",      consultorName:"Fabi RH" },
  { username:"marcelo",  password:"marcelo2025",role:"consultor", label:"Consultor",      consultorName:"Marcelo Franco" },
  { username:"luciano",  password:"luciano2025",role:"consultor", label:"Consultor",      consultorName:"Luciano Trigilio" },
  { username:"heverson", password:"heverson2025",role:"consultor",label:"Consultor",      consultorName:"Heverson Gomes" },
  { username:"thiago",   password:"thiago2025", role:"consultor", label:"Consultor",      consultorName:"Thiago Berna" },
  { username:"rodolfo",  password:"rodolfo2025",role:"consultor", label:"Consultor",      consultorName:"Rodolfo Rosseto" },
];
      ...
      <div style={{ width:"100%", maxWidth:"420px" }}>
        ...
        <div style={{ background:"#1e293b", borderRadius:"16px", padding:"32px", border:"1px solid #334155", boxShadow:"0 20px 60px rgba(0,0,0,0.4)" }}>
          <div style={{ display:"flex", flexDirection:"column", gap:"16px" }}>
            <div>
              <label style={{ fontSize:"12px", color:"#64748b", fontWeight:600, display:"block", marginBottom:"6px" }}>Usuário</label>
              <input value={username} onChange={e=>{ setUsername(e.target.value); setError(""); }} onKeyDown={e=>e.key==="Enter"&&handleSubmit()} placeholder="Digite seu usuário..." style={inp} autoFocus />
            </div>
            <div>
            ...
            {error && (
              <div style={{ padding:"10px 14px", borderRadius:"8px", background:"#ef444422", border:"1px solid #ef444444", color:"#ef4444", fontSize:"13px", fontWeight:500 }}>⚠️ {error}</div>
            )}
            <button onClick={handleSubmit} disabled={loading} style={{ padding:"12px", borderRadius:"8px", border:"none", background: loading?"#334155":"#3b82f6", color: loading?"#64748b":"#fff", fontWeight:700, fontSize:"15px", cursor: loading?"not-allowed":"pointer", marginTop:"4px", transition:"background 0.2s" }}>
        ...
        {/* Hint card */}
        <div style={{ marginTop:"24px", background:"#1e293b88", borderRadius:"12px", padding:"16px 20px", border:"1px solid #1e293b" }}>
          <p style={{ fontSize:"12px", color:"#475569", fontWeight:600, margin:"0 0 10px" }}>🔑 Acessos de demonstração</p>
          <div style={{ display:"flex", flexDirection:"column", gap:"5px" }}>
            {[["admin","admin123","Admin — acesso total"],["editor","editor123","Editor — edita agendas"],["viewer","viewer123","Visualizador — só leitura"],["antonio","antonio2025","Consultor — só sua agenda"]].map(([u,p,desc])=>(
              <div key={u} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"5px 8px", borderRadius:"6px", background:"#0f172a", cursor:"pointer" }}
                onClick={()=>{ setUsername(u); setPassword(p); setError(""); }}>
                <span style={{ fontSize:"12px", color:"#64748b" }}>{desc}</span>
                <span style={{ fontSize:"11px", color:"#334155", fontFamily:"monospace" }}>{u} / {p}</span>
              </div>
            ))}
          </div>
          <p style={{ fontSize:"11px", color:"#334155", margin:"10px 0 0" }}>Clique numa linha para preencher automaticamente</p>
        </div>
      </div>
    </div>
  );
...
function Dashboard({ currentUser, onLogout }) {
  ...
  // For consultor role: lock to their own name; otherwise use filter selection
  ...
  const allMonths = useMemo(()=>{ const seen=new Set(),ordered=[]; MONTHS_ORDER.forEach(m=>{if(!seen.has(m)){seen.add(m);ordered.push(m);}}); return ["Todos",...ordered]; },[]);
    ...
    <div style={{ fontFamily:"'DM Sans',sans-serif",background:"#0f172a",minHeight:"100vh",color:"#e2e8f0" }}>
      ...
      {view !== "cadastros" && (
        <div style={{ background:"#1e293b",padding:"12px 32px",display:"flex",gap:"16px",flexWrap:"wrap",alignItems:"center",borderBottom:"1px solid #334155" }}>
          {!isConsultor && (
            <div style={{ display:"flex",gap:"8px",overflowX:"auto",paddingBottom:"4px",flex:"1 1 300px" }}>
              <button onClick={()=>setSelectedConsultor(null)} style={{ whiteSpace:"nowrap",padding:"6px 14px",borderRadius:"20px",border:"none",cursor:"pointer",fontWeight:600,fontSize:"12px",background:!selectedConsultor?"#3b82f6":"#334155",color:!selectedConsultor?"#fff":"#94a3b8" }}>Todos</button>
              {consultores.map(c=>(
                <button key={c} onClick={()=>setSelectedConsultor(selectedConsultor===c?null:c)} style={{ whiteSpace:"nowrap",padding:"6px 14px",borderRadius:"20px",border:"none",cursor:"pointer",fontWeight:600,fontSize:"12px",background:selectedConsultor===c?"#3b82f6":"#334155",color:selectedConsultor===c?"#fff":"#94a3b8",display:"flex",alignItems:"center",gap:"6px" }}>
                  <span style={{ width:"18px",height:"18px",borderRadius:"50%",background:"hsl("+(consultores.indexOf(c)*29%360)+",65%,55%)",display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:"8px",fontWeight:700,color:"#fff" }}>{getInitials(c)}</span>
                  {c.split(" ")[0]}
                </button>
              ))}
            </div>
